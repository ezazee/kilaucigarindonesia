...
export interface AboutContent {
  ...
  nicaraguaTitle: string;
  nicaraguaDesc: string;
...
export default function AboutPageClient({ content, images }: { content: AboutContent; images: AboutImages }) {
    ...
    <div className="min-h-screen bg-[#050505] text-white overflow-x-hidden">
      ...
      <section className="relative h-[80vh] flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 text-center space-y-8 max-w-5xl px-6">
          <div className="space-y-4 animate-fade-in">
            <h1 className="text-secondary text-[11px] font-bold uppercase tracking-[0.6em] mb-4">{content.badge}</h1>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-24 items-start">
            ...
            <div className="space-y-16 animate-slide-right order-1 lg:order-2">
              <div className="space-y-6">
                <h3 className="text-secondary text-xs font-bold uppercase tracking-[0.4em]">{content.historyBadge}</h3>
                <h4 className="text-4xl md:text-6xl font-serif font-bold leading-tight">
                  {content.historyTitle} <br />
                  <span className="italic">{content.historySubtitle}</span>
                </h4>
              </div>

              <div className="space-y-10 text-zinc-400 font-light leading-[1.8] text-lg">
                <div className="space-y-4">
                  <p>{content.historyDesc1}</p>
                  <p>{content.historyDesc2}</p>
                </div>

                <div className="space-y-4 pt-8 border-t border-white/5">
                  <h5 className="text-white font-serif italic text-2xl">{content.experienceTitle}</h5>
                  <p>{content.experienceDesc}</p>
                </div>

                <div className="space-y-4 pt-8 border-t border-white/5">
                  <h5 className="text-white font-serif italic text-2xl">{content.nicaraguaTitle}</h5>
                  <p>{content.nicaraguaDesc}</p>
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="flex-1 space-y-10">
              <div className="space-y-6">
                <h3 className="text-4xl md:text-6xl font-serif font-bold leading-tight uppercase">
                  {content.innovationTitle} <br />
                  <span className="text-secondary italic">{content.innovationSubtitle}</span>
                </h3>
      ...
      <section className="py-48 relative overflow-hidden text-center bg-[#050505]">
        ...
        <div className="max-w-5xl mx-auto px-8 relative z-10 space-y-12">
          <div className="text-secondary text-6xl font-serif">&ldquo;</div>
          ...
          <p className="text-zinc-500 uppercase tracking-[0.5em] text-[10px] font-bold">{content.heritage}</p>
        </div>
      </section>
        ...
        <div className="max-w-4xl mx-auto px-8 space-y-12">
          <div className="flex flex-col items-center space-y-6">
             ...
             <h3 className="text-4xl md:text-5xl font-serif font-bold">
                {content.qualityTitle} <br />
                <span className="text-secondary">{content.qualitySubtitle}</span>
             </h3>
          </div>
